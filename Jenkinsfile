      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'sudo docker tag nodeapp-dev:trunk orkumashiaondo/nodeapp-dev:latest'
            sh 'sudo docker push orkumashiaondo/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'sudo docker tag nodeapp-dev:trunk orkumashiaondo/nodeapp-prod:latest'
                            sh 'sudo docker push orkumashiaondo/nodeapp-prod:latest'
                            sh 'sudo docker save orkumashiaondo/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
