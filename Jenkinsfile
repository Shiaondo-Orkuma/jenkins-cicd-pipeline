      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk orkumashiaondo/nodeapp-dev:latest'
            sh 'docker push orkumashiaondo/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk orkumashiaondo/nodeapp-prod:latest'
                            sh 'docker push orkumashiaondo/nodeapp-prod:latest'
                            sh 'docker save orkumashiaondo/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
